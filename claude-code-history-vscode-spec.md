...
> - **Hebrew-quality finding (LOW priority, deferred):** with `unicode61 remove_diacritics 2`, `MATCH 'שלום'` did **not** also match a niqqud-pointed row (`שָׁלוֹם`); Hebrew points (U+05B0–U+05BF) aren't folded by this tokenizer/SQLite build. **Deprioritized per author** — niqqud is rare in chat transcripts. If revisited, fix via pre-normalization (strip niqqud before indexing) in one shared place since it affects the CLI too and the byte-for-byte parity contract.
